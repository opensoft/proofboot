...
travis_time_start;
echo -e "\033[1;33mRemoving tests, examples and android stuff...\033[0m";
rm -rf proof-bin/tools proof-bin/tests proof-bin/examples proof-bin/android;
travis_time_finish;
travis_time_start;
...
if [ -n "$RELEASE_BUILD" ]; then
    aws s3 cp proof-bin.tar.gz s3://proof.travis.builds/__releases/proof/raw-bin/$PROOF_VERSION/proof-bin-debian9.tar.gz;
else
    aws s3 cp proof-bin.tar.gz s3://proof.travis.builds/$TRAVIS_BRANCH/proof-bin-debian9.tar.gz;
fi
travis_time_finish
