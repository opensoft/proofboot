...
docker run -id --name builder -v $(pwd):/sandbox/proof -w="/sandbox" opensoftdev/proof-builder-base tail -f /dev/null;
docker ps;
...
echo "$ proof/deploy/deb/build-deb-package -f /sandbox/proof/deploy/deb/Manifest.dev /sandbox/proof";
docker exec -t builder bash -c "proof/deploy/deb/build-deb-package -f /sandbox/proof/deploy/deb/Manifest.dev /sandbox/proof";
...
if [ -n "$RELEASE_BUILD" ]; then
    proofboot/travis/s3_upload.sh proof-dev-$PROOF_VERSION.deb __releases/proof proof-dev-$PROOF_VERSION.deb;
else
    proofboot/travis/s3_upload.sh proof-dev-$PROOF_VERSION.deb $TRAVIS_BRANCH proof-dev-$PROOF_VERSION-$TRAVIS_BRANCH.deb;
fi
travis_time_finish
